...
## Resume training, validation, and testing of a model
In some cases, a model is trained for a certain number of epochs. The hyper-parameters are then changed, and training resumes. This is done by first training a model by using the previous command. Then, resume training with the following command:
```
